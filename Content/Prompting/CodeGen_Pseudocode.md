    ...
    **Steps to Implement in Pseudocode**:
    ...
    1. **Interpret the Graph(s)**  
       You may encounter one or more graphs in the `"graphs"` array. Each graph might represent:
        ...
        - A **Macro** graph (`graph_type = "Macro"`)
       ...
       **Handling Multiple Graphs**:
       ...
       2. **Translate Each Node to Pseudocode**:
           ...
           - Reflect any loops (e.g., ForLoop, WhileLoop, ForEachLoop, etc.) or branches (e.g., Branch node -> IF) in proper CalPoly pseudocode constructs (`WHILE`, `IF-THEN-ELSE`, `CASE`, `FOR`, `REPEAT-UNTIL`).
       ...
       5. **Handling Different Node Properties**:
           - `"pure": true` means the node has no execution pins; it’s a pure function. In pseudocode, you can embed it as an expression or sub-procedure call in line with your logic.
           - `"latent": true` means the node is asynchronous. Mention any relevant note, but typically your pseudocode can just say `CALL SomeAsyncProcess`.
    
       6. **Adherence to Pseudocode Standard**:
           - Use **CalPoly Pseudocode Standard** constructs: `SEQUENCE`, `WHILE`, `IF-THEN-ELSE`, `REPEAT-UNTIL`, `FOR`, `CASE`.
           - Indent nested blocks.
           - Use `CALL` for sub-procedures.
           - Use `IF`, `THEN`, `ELSE`, `ENDIF`; `WHILE`, `ENDWHILE`; `REPEAT`, `UNTIL`; `FOR`, `ENDFOR`; `CASE`, `OF`, `OTHERS`, `ENDCASE` as described.
           - Provide minimal but clear variable references and comment as needed.
    
       7. **No Additional Explanations**:
           - Your final output must be strictly JSON with a **"graphs"** property containing an array of graph objects (each describing one pseudocode translation). No extra text or discussion outside the JSON structure.
    ...
    **Field Requirements**:
    ...
    1. **graph_name**: The name of the graph or function (taken from the N2C JSON’s `"name"` field).
       ...
       4. **code**: An object holding three string fields:
           - **graphDeclaration**: This will be left empty.
           - **graphImplementation**: The step-by-step pseudocode block for the logic inside the graph, within **Markdown** code markers.
           - **implementationNotes**: Any extra clarifying details about the logic or special considerations.
